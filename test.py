...
class DataScraper:
        ...
        for en in all_en_list:
            ch = "".join(
                [
                    i.strip()
                    for i in res.xpath(
                        f'//div[@class="dataview"]//table[1]//th[@data-field="{en}"]//text()'
                    )
                ]
            )
    ...
    def get_timeList(self):
        # 定义一个请求头，用于模拟浏览器请求
        headers = {"Referer": "https://data.eastmoney.com/bbsj/202312.html"}
        # 使用 requests 库发送 GET 请求，获取页面数据
        response = requests.get(
            "https://data.eastmoney.com/bbsj/202312.html", headers=headers
        )
    ...
    def run(self):
        ...
        self.pagename = str(
            input(
                "请输入报表类型（业绩报表;业绩快报;业绩预告;预约披露时间;资产负债表;利润表；现金流量表）:"
            )
        )
