from selenium_getdata import webdriver
...
def index_page(page):
    try:
        ...
        if page > 1:
            # 确定页数输入框
            input = wait.until(
                EC.presence_of_element_located((By.XPATH, '//*[@id="PageContgopage"]'))
            )
            input.click()
            input.clear()
            input.send_keys(page)
            submit = wait.until(
                EC.element_to_be_clickable((By.CSS_SELECTOR, "#PageCont > a.btn_link"))
            )
            submit.click()
            time.sleep(2)
        # 确认成功跳转到输入框中的指定页
...
def write_to_file(df_table, category):
    ...
    os.chdir(file_path)
    df_table.to_csv(
        "{}.csv".format(category), mode="a", encoding="utf_8_sig", index=0, header=0
    )
...
def set_table():
    ...
    quarter = int(
        float(input("请输入小写数字季度(1:1季报，2-年中报，3：3季报，4-年报)：\n"))
    )
