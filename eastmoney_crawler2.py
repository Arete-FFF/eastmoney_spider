...
file_path = "D:\\code\\python\\eastmoney_spider\\out"
...
def set_table():
    ...
    # 1 设置财务报表获取时期
    year = int(float(input("请输入要查询的年份(四位数2007-2023)：\n")))
    # int表示取整，里面加float是因为输入的是str，直接int会报错，float则不会
    # https://stackoverflow.com/questions/1841565/valueerror-invalid-literal-for-int-with-base-10
    while year < 2007 or year > 2024:
...
def page_choose(page_all):
    ...
    # 返回所需的起始页数，供后续程序调用
    yield {"start_page": start_page, "end_page": end_page}
...
def write_table(data, page, category):
    ...
    for d in data:
        with open(
            "{}.csv".format(category), "a", encoding="utf_8_sig", newline=""
        ) as f:
            w = csv.writer(f)
            w.writerow(d.values())
